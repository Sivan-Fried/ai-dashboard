...
def show_tasks_page(project_name=None):
    ...
    def calc_status(row):
        if row["status"] == "הושלם":
            return "הושלם"
        try:
            if row["due_date"].date() < today and row["status"] != "הושלם":
                return "באיחור"
        except:
            pass
        return row["status"]
    ...
    with st.container(border=True):
        st.markdown("""
        <div style="display:grid;grid-template-columns:2.5fr 1fr 1fr 0.8fr 0.8fr;
            gap:8px;padding:10px 16px;background:#f8fafc;border-radius:10px;
            margin-bottom:4px;direction:rtl;text-align:right;">
            <span style="font-size:0.75rem;font-weight:700;color:#94a3b8;">תיאור</span>
            <span style="font-size:0.75rem;font-weight:700;color:#94a3b8;">סטטוס</span>
            <span style="font-size:0.75rem;font-weight:700;color:#94a3b8;">אחראי</span>
            <span style="font-size:0.75rem;font-weight:700;color:#94a3b8;">התחלה</span>
            <span style="font-size:0.75rem;font-weight:700;color:#94a3b8;">יעד</span>
        </div>
        """, unsafe_allow_html=True)
        ...
        if df_view.empty:
            ...
            for _, row in df_view.iterrows():
                ...
                notes_html = f"<div style='font-size:0.7rem;color:#a1a1aa;margin-top:2px;'>{notes}</div>" if notes and notes != "nan" else ""
                due_color = "#ef4444" if s == "באיחור" else "#71717a"
                st.markdown(f"""
                <div style="display:grid;grid-template-columns:2.5fr 1fr 1fr 0.8fr 0.8fr;
                    gap:8px;padding:12px 16px;border-bottom:1px solid #f4f4f5;
                    direction:rtl;text-align:right;align-items:center;">
                    <span>
                        <div style="font-size:0.82rem;font-weight:600;color:#3f3f46;">{row['description']}</div>
                        {notes_html}
                    </span>
                    <span>
                        <span style="display:inline-block;padding:3px 10px;border-radius:20px;
                            font-size:0.72rem;font-weight:700;
                            background:{sc['bg']};color:{sc['color']};">{s}</span>
                    </span>
                    <span style="font-size:0.82rem;color:#3f3f46;">{row.get('responsible','')}</span>
                    <span style="font-size:0.78rem;color:#71717a;">{fmt_date(row.get('start_date',''))}</span>
                    <span style="font-size:0.78rem;color:{due_color};">{fmt_date(row.get('due_date',''))}</span>
                </div>
                """, unsafe_allow_html=True)
