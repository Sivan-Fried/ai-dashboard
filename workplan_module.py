...
# ---------------------------------------------------------
def milestone_to_html(row, right_px=0):
    ...
    return (
        f"<div class='item' data-date='{date_iso}' style='right:{right_px}px;'>"
        "<div class='card'>"
        + tooltip_html +
...
# ---------------------------------------------------------
def build_items_html(project_df, timeline_start, total_days, timeline_width):
    items = []
    for _, row in project_df.iterrows():
        days = (row["date"].date() - timeline_start).days
        days = max(0, min(days, total_days))
        right_px = int(timeline_width - (days / total_days * timeline_width)) + 50 - 45
        items.append(milestone_to_html(row, right_px=right_px))
    return "\n".join(items)
...
<head>
    ...
    <style>
        ...
        .timeline-wrapper { position: relative; margin: 30px 0; height: 200px; min-width: 1000px; }
        ...
        .item { display: flex; flex-direction: column; align-items: center; width: 90px; z-index: 3; position: absolute; bottom: 0; }
...
def build_timeline_html(project_name):
    ...
    items_html = build_items_html(project_df, timeline_start, total_days, timeline_width)
