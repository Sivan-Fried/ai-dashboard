...
# --- אזור Fathom: ביצועים מקסימליים ללא Rerun ---
with st.container(border=True):
    st.markdown("### ✨ סיכומי פגישות Fathom")
    
    # CSS להידוק ועיצוב ה-Expander שייראה כמו השורות המקוריות שלך
    st.markdown("""
        <style>
        /* עיצוב ה-Expander שייראה כמו שורה מהודקת */
        .stExpander {
            border: 1px solid #edf2f7 !important;
            border-right: 5px solid #4facfe !important;
    ...
    if meetings:
        for idx, mtg in enumerate(meetings):
            ...
            # השימוש ב-Expander מבטל את הצורך ב-Rerun ובלחיצה כבדה
            with st.expander(f"📅 **{title}** &nbsp; <span class='f-pill'>{date}</span>", expanded=False):
                s_key = f"sum_v4_{r_id}"
                if s_key not in st.session_state:
                    if st.button("צור סיכום עם AI 🪄", key=f"gen_{r_id}"):
