...
# 2. כותרת, תמונה וברכה (החזרת המבנה המקורי)
...
# בניית חלק עליון: תמונה במרכז, ברכה מימין
st.markdown("<div style='text-align:center; margin-bottom:10px;'><h2 style='font-weight:800; direction:rtl;'><span class='text-gradient'>Dashboard AI</span> <span style='color: #1f2a44;'>לניהול פרויקטים</span></h2></div>", unsafe_allow_html=True)

col_info, col_img, col_spacer = st.columns([1.5, 1, 1.5])

with col_info:
    # ברכה ושעה - מיושר לימין
    st.markdown(f"""
...
with k1: st.markdown(f"<div class='kpi-container'><p style='color:gray; margin:0;'>סה״כ פרויקטים</p><h3 style='margin:5px 0;'>{len(projects)}</h3></div>", unsafe_allow_html=True)
with k2: st.markdown(f"<div class='kpi-container' style='border-top:3px solid red;'><p style='color:gray; margin:0;'>בסיכון 🔴</p><h3 style='color:red; margin:5px 0;'>{len(projects[projects['status']=='אדום'])}</h3></div>", unsafe_allow_html=True)
with k3: st.markdown(f"<div class='kpi-container' style='border-top:3px solid orange;'><p style='color:gray; margin:0;'>במעקב 🟡</p><h3 style='color:orange; margin:5px 0;'>{len(projects[projects['status']=='צהוב'])}</h3></div>", unsafe_allow_html=True)
with k4: st.markdown(f"<div class='kpi-container' style='border-top:3px solid green;'><p style='color:gray; margin:0;'>לפי התכנון 🟢</p><h3 style='color:green; margin:5px 0;'>{len(projects[projects['status']=='ירוק'])}</h3></div>", unsafe_allow_html=True)
...
with col_l:
    ...
    if st.session_state.get("add_mode"):
        with st.form("add_rem"):
            ...
            prj = st.selectbox("פרויקט", projects["project_name"].tolist())
...
with ca2: q_in = st.text_input("שאלה לבוט", key="q_ai")
if st.button("נתח"): st.info("מנתח נתוני פרויקט...")
st.markdown("</div>", unsafe_allow_html=True)
