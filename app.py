...
# שימוש ב-Container פנימי של Streamlit כדי לשמור על יציבות
with st.container():
...
with col_r:
    ...
    for _, row in today_m.iterrows():
        st.markdown(f"<div style='background:white; padding:15px; border-radius:10px; border:1px solid #eee; margin-bottom:10px; text-align:right;'>📌 <b>{row['meeting_title']}</b><br><small>{row['time']} | {row['project_name']}</small></div>", unsafe_allow_html=True)
...
with col_l:
    ...
    with st.container(height=280):
        for _, row in today_r.iterrows():
            st.markdown(f"<div style='background:white; padding:15px; border-radius:10px; border:1px solid #eee; margin-bottom:10px; text-align:right;'>🔔 {row['reminder_text']} | {row['project_name']}</div>", unsafe_allow_html=True)
...
# 7. AI Oracle
st.markdown("<div class='outer-section' style='border-right: 6px solid #4facfe;'><h3>✨ AI Oracle</h3>", unsafe_allow_html=True)
