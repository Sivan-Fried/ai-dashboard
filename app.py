...
<style>
    ...
    /* הסתרת הפס הלבן והשטח הריק של רכיבי ה-JS */
    iframe { display: none; }
...
# =========================================================
# 2. לוגיקה ופונקציות (Weather, Azure, Fathom)
# =========================================================
...
def get_real_weather(loc_data):
    if loc_data and 'coords' in loc_data:
        try:
            ...
            city = g.get('address', {}).get('city') or g.get('address', {}).get('town') or "המיקום שלי"
...
def get_azure_tasks():
    try:
        ...
        res = requests.post(f"https://dev.azure.com/amandigital/_apis/wit/wiql?api-version=6.0", json={"query": "SELECT [System.Id], [System.Title], [System.TeamProject] FROM WorkItems WHERE [System.AssignedTo] = @me AND [System.State] = 'New'"}, auth=auth)
        ids = ",".join([str(i['id']) for i in res.json().get('workItems', [])[:5]])
        if not ids: return []
        return requests.get(f"https://dev.azure.com/amandigital/_apis/wit/workitems?ids={ids}&fields=System.Title,System.TeamProject,System.CreatedDate&api-version=6.0", auth=auth).json().get('value', [])
    except: return []
        ...
        with st.container(border=True):
            ...
            for _, row in projects.iterrows():
                if st.button(f"📂 {row['project_name']}", key=f"p_{row['project_name']}", use_container_width=True):
                    st.session_state.selected_project = row['project_name']; st.session_state.current_page = "project"; st.rerun()
    ...
    with c_left:
        with st.container(border=True):
            ...
            for _, r in t_m.iterrows():
                st.markdown(f'<div class="record-row"><span>📌 {r["meeting_title"]}</span><span class="time-label">{r["start_time"]}</span></div>', unsafe_allow_html=True)

        with st.container(border=True):
            st.markdown("### ✨ Fathom AI")
            for mtg in get_fathom_meetings():
                with st.expander(f"📅 {mtg['title']}"):
                    if st.button("סכם פגישה", key=f"f_{mtg['recording_id']}"):
                        st.write(refine_with_ai(get_fathom_summary(mtg['recording_id'])))
