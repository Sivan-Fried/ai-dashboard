        ...
        with st.container(border=True):
            ...
            if f_mtgs:
                for idx, mtg in enumerate(f_mtgs):
                    ...
                    st.markdown(f'''<div class="fathom-row-ui"><div>📅 {title} <span class="fathom-pill-v2">{date_str}</span></div><div></div><span class="material-symbols-rounded">{arrow}</span></div>''', unsafe_allow_html=True)
