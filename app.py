...
<style>
    ...
    /* ביטול עיצוב ברירת מחדל של כפתורי Streamlit שמשמשים כשורות לחיצות */
...
# =========================================================
# 2. פונקציות עזר (Azure, Fathom, AI)
# =========================================================
...
def get_fathom_meetings():
    ...
    try:
        ...
        return (res.json().get('items', [])[:5], 200) if res.status_code == 200 else (res.text, res.status_code)
...
if st.session_state.current_page == "project":
    ...
    with st.container(border=True):
        ...
        with tab_work:
            ...
            else: st.info(f"תוכנית עבודה עבור {p_name}")
    ...
    with col_right:
        ...
        with st.container(border=True):
            ...
            with st.container(height=300, border=False):
                for _, row in projects.iterrows():
                    ...
                    '''
                    if st.button(row_html, key=f"btn_p_{p_name}", unsafe_allow_html=True):
        ...
        with st.container(border=True):
            ...
            if tasks:
                for t in tasks:
                    f = t.get('fields', {})
                    t_url = f"https://dev.azure.com/amandigital/{urllib.parse.quote(f.get('System.TeamProject',''))}/_workitems/edit/{t.get('id')}"
                    st.markdown(f'<div class="record-row"><a href="{t_url}" target="_blank" style="text-decoration:none; color:#0078d4;">🔗 {f.get("System.Title")}</a><span class="tag-orange">{f.get("System.TeamProject")}</span></div>', unsafe_allow_html=True)
    ...
    with col_left:
        ...
        with st.container(border=True):
            ...
            for mtg in st.session_state.f_mtgs:
                ...
                f_html = f'<div class="record-row"><div style="display:flex;align-items:center;gap:10px;"><b>📅 {mtg.get("title")}</b></div><span class="material-symbols-rounded" style="color:#94a3b8;font-size:20px;">{arrow}</span></div>'
                
                if st.button(f_html, key=f"f_btn_{rid}", unsafe_allow_html=True):
