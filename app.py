...
# 4. KPI - עיצוב מקורי (3 עמודות) עם חישוב מדויק
c1, c2, c3 = st.columns(3)
with c1:
    st.markdown(f"<div class='kpi-box'><b>סה״כ פרויקטים</b><br>{len(projects)}</div>", unsafe_allow_html=True)
with c2:
    red_count = len(projects[projects['status'] == 'אדום'])
    st.markdown(f"<div class='kpi-box'><b>בסיכון 🔴</b><br>{red_count}</div>", unsafe_allow_html=True)
with c3:
    # כאן אנחנו סופמים את כל מה שלא אדום (צהוב + ירוק/לפי התכנון) כדי להשלים ל-100%
    other_count = len(projects[projects['status'] != 'אדום'])
    st.markdown(f"<div class='kpi-box'><b>במעקב/תקין 🟢🟡</b><br>{other_count}</div>", unsafe_allow_html=True)

st.markdown("<br>", unsafe_allow_html=True)

# 5. פריסה: פרויקטים בימין, פגישות ותזכורות בשמאל
...
with col_right:
    ...
    for _, row in projects.iterrows():
        ...
        st.markdown(f"""
        <div style="background:white; padding:8px 12px; border-radius:8px; margin-bottom:5px; border:1px solid #eee; direction:rtl; text-align:right; font-size:14px;">
...
with col_left:
    ...
    if today_meetings.empty:
        ...
        for _, row in today_meetings.iterrows():
            st.markdown(f"<div class='card'>📌 {row['meeting_title']}<br><span style='font-size:12px; color:gray;'>🕒 {row['time']} | 📁 {row['project_name']}</span></div>", unsafe_allow_html=True)
...
if api_key:
    ...
    with ca1:
        s_proj = st.selectbox("בחרי פרויקט לניתוח", projects["project_name"].tolist(), key="ai_sel")
    with ca2:
        u_q = st.text_area("מה תרצי לדעת?", placeholder="למשל: מה הסטטוס הנוכחי?", key="ai_txt")

    if st.button("בצע ניתוח AI"):
        if u_q:
            p_info = projects[projects["project_name"] == s_proj].iloc[0]
            context = f"פרויקט: {p_info.to_string()}. ענה בעברית תמציתית מאוד."
            ...
            except: st.error("שגיאת תקשורת עם ה-AI")
