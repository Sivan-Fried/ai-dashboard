...
with p2:
    if img_b64:
        st.markdown(f'<div style="display:flex; justify-content:center;"><img src="data:image/png;base64,{img_b64}" style="width:120px; height:120px; border-radius:50%; border:3px solid white; object-fit: cover;"></div>', unsafe_allow_html=True)
with p3:
...
with col_right:
    with st.container(border=True):
        ...
        for i, row in projects.iterrows():
            st.markdown(f'<div class="record-box"><span><b>{row["project_name"]}</b></span><span style="color:gray;">{row.get("project_type", "")}</span></div>', unsafe_allow_html=True)

    with st.container(border=True):
        st.markdown("### ✨ AI Oracle")
        a1, a2 = st.columns([1, 2])
        with a1: st.selectbox("בחר פרויקט", projects["project_name"].tolist(), label_visibility="collapsed", key="sel_unique_1")
        with a2: st.text_input("שאלה", placeholder="מה תרצי לדעת?", label_visibility="collapsed", key="input_unique_1")
        st.button("שגר שאילתה 🚀", use_container_width=True, key="btn_unique_1")
...
with col_left:
    ...
    with st.container(border=True):
        ...
        if st.button("➕ הוסף תזכורת", key="btn_add_rem"): st.session_state.add_mode = True
        if st.session_state.get("add_mode"):
            nt = st.text_input("משימה:", key="new_task_val")
            if st.button("✅ שמור", key="btn_save_rem"):
