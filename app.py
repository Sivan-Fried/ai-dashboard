...
# --- 2. CSS "אלים" שעוקף את המערכת של Streamlit ---
st.markdown("""
<style>
    /* רקע כללי */
    .stApp { background-color: #f2f4f7 !important; direction: rtl !important; }

    /* כותרת גרדיאנט - דריסה מוחלטת */
    .dashboard-header {
    ...
    /* כפיית מסגרות גרדיאנט על כל מכולה עם border=True */
    ...
    /* עיצוב הרשומות (התיבות הלבנות עם הפס התכלת) */
    .record-box {
        ...
        border-right: 6px solid #4facfe !important; /* תכלת אחיד לכולן */
...
with col_r:
    ...
    with st.container(border=True):
        ...
        with a1: st.selectbox("בחר פרויקט", projects["project_name"].tolist(), label_visibility="collapsed", key="ai_sel_fixed")
        with a2: st.text_input("שאלה", placeholder="מה תרצי לדעת?", label_visibility="collapsed", key="ai_in_fixed")
...
with col_l:
    ...
    with st.container(border=True):
        ...
        if today_meetings.empty: st.write("אין פגישות היום")
        else:
            for _, r in today_meetings.iterrows():
                st.markdown(f'<div class="record-box"><span>📌 {r["meeting_title"]}</span></div>', unsafe_allow_html=True)

    # אזור תזכורות
    with st.container(border=True):
        st.markdown("### 🔔 תזכורות")
        today_rems = st.session_state.rem_live[pd.to_datetime(st.session_state.rem_live["date"]).dt.date == today]
        with st.container(height=200, border=False):
            for _, row in today_rems.iterrows():
        ...
        if st.session_state.get("add_task"):
            nt = st.text_input("משימה חדשה:", key="new_task_input")
            if st.button("✅ שמור"):
