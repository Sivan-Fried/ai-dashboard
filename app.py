...
# --- 2. CSS אבסולוטי - תיקון מסגרות ורשומות ---
st.markdown("""
<style>
    ...
    /* תיקון מסגרות הגרדיאנט ל-Containers */
    ...
    /* עיצוב רשומות פרויקטים (שם + אייקון/סוג) */
    .project-record {
        ...
        for _, row in projects.iterrows():
            st.markdown(f"""
                <div class="project-record">
                    <span style="font-weight:bold; font-size:1.1em;">📂 {row['project_name']}</span>
                    <span style="color:#4facfe; font-weight:500;">{row.get('project_type', 'פרויקט')}</span>
                </div>
            """, unsafe_allow_html=True)
...
with col_left:
    ...
    with st.container(border=True):
        ...
        if t_m.empty: st.write("אין פגישות היום")
            ...
            for _, r in t_m.iterrows():
                st.markdown(f'<div class="project-record"><span>📌 {r["meeting_title"]}</span></div>', unsafe_allow_html=True)
    ...
    with st.container(border=True):
        ...
        for _, row in t_r.iterrows():
            st.markdown(f'<div class="project-record"><span>🔔 {row["reminder_text"]}</span></div>', unsafe_allow_html=True)
