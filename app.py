...
# ייבוא הקוד של התרשים מהמודול השני
...
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
# התחלת אזור חדש: ניהול פרויקט מפורט עם תרשים
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
...
if st.session_state.current_page == "project":
    ...
    with st.container(border=True):
        ...
        with tab_work:
            # הטמעת התרשים מהמודול (הקוד שהיה ב-altshuler_module.py)
            roadmap_html = """
            <!DOCTYPE html>
...
# <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
# סוף אזור חדש: ניהול פרויקט מפורט עם תרשים
# <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
