...
if st.session_state.current_page == "project":
    ...
    with col_left:
        ...
        # --- אזור Fathom ---
        with st.container(border=True):
            c_t, c_r = st.columns([0.9, 0.1])
            c_t.markdown("### ✨ סיכומי Fathom")
            if c_r.button("🔄", key="ref_f"):
            ...
            for idx, mtg in enumerate(st.session_state.get('fathom_meetings', [])):
                ...
                st.markdown(f'''<div class="fathom-row-ui"><div style="display:flex;align-items:center;"><span style="margin-left:10px;">📅</span><span style="font-weight:600;font-size:0.85rem;">{mtg.get('title')}</span><span class="fathom-pill-v2">{mtg.get('recording_start_time')[:10]}</span></div><div></div><span class="material-symbols-rounded" style="color:#94a3b8;font-size:20px;">{"expand_more" if is_open else "chevron_left"}</span></div>''', unsafe_allow_html=True)
