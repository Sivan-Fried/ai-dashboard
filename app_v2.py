...
if st.session_state.current_page == "project":
    ...
    try:
        # טעינת הקובץ
        quotes_df = pd.read_excel("inspirational_quotes.xlsx")
        # בחירת שורה רנדומלית מתוך העמודות quote ו-author
        random_row = quotes_df.sample(n=1).iloc[0]
        quote_text = random_row['quote']
        quote_author = random_row['author']
    except Exception as e:
        # אם הקובץ לא נמצא או שיש שגיאה בעמודות
        quote_text = "המסע היחיד הוא זה שבפנים."
        quote_author = "לא ידוע"
        # הדפסת השגיאה לטרמינל כדי שתוכלי לדבג
        print(f"Error loading quotes: {e}")
    
    # ── תצוגת ה-HTML (גובה מינימלי ומבנה נקי) ─────────────────────
    quote_content = f"""
