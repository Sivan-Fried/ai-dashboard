...
if st.session_state.current_page == "project":
    ...
    # 1. הגדרת המיקום - הורדנו ל-35- כדי שירד קצת למטה מהסרגל
    st.markdown("""
    <style>
        .block-container { padding-top: 0rem !important; }
        
        .quote-final-container {
    ...
    # 2. ה-HTML המקורי עם כל העיצוב, הפונטים והאייקונים
    html_design_fixed = f"""
    <div style="direction: rtl; background: #ffffff; background-image: radial-gradient(circle at 15% 50%, rgba(250, 220, 230, 0.4) 0%, transparent 45%), radial-gradient(circle at 85% 80%, rgba(227, 225, 236, 0.4) 0%, transparent 45%); border-bottom: 1px solid #f1f5f9; padding: 15px 20px; text-align: center; font-family: 'Plus Jakarta Sans', sans-serif;">
        ...
        <div style="font-family: 'Noto Serif Hebrew', serif; font-size: 22px; color: #1a1c1c; line-height: 1.2; margin-bottom: 6px; font-weight: 700;">
            "{quote_text}"
        </div>
        
        <div style="font-size: 13px; color: #646566; font-style: italic; margin-bottom: 12px;">
            &#8212; {quote_author} &#8212;
        </div>
        
        <div style="display: flex; align-items: center; justify-content: center; gap: 10px; flex-direction: row;">
    ...
    # 3. הצגת הרכיב בתוך המעטפת ששולטת במיקום
    st.markdown('<div class="quote-final-container">', unsafe_allow_html=True)
    st.components.v1.html(f'<body style="margin:0; padding:0;">{html_design_fixed}</body>', height=140)
